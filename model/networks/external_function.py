...
class PerceptualCorrectness(nn.Module):
    ...
    def __call__(self, target, source, flow_list, used_layers, mask=None, use_bilinear_sampling=False):
        ...
        loss = 0
        for i in range(len(flow_list)):
            loss += self.calculate_loss(flow_list[i], 'relu4_1', mask, use_bilinear_sampling)
        return loss
...
class CorrelationLoss(nn.Module):
    ...
    def __call__(self, x, y, source):
        ...
        return content_loss, style_loss, correlation_loss*500





class Correctness(nn.Module):
    r"""
    ...
    def calculate_loss(self):
        ...
        target_all = target.view(b, c, -1)                      #[b C N2]
        source_all = source.view(b, c, -1).transpose(1,2)       #[b N2 C]
        source_cal = source.view(b, c, -1)
        ...
        correction_sample = F.cosine_similarity(source_cal, target_all)    #[b 1 N2]
